...
class AuthController extends Controller
{
    ...
    public function completeProfile(Request $request)
    {
        ...
        } elseif ($userPhoneFormatted !== $phone || !$request->whatsapp_verified) {
        ...
        if ($request->whatsapp_verified || ($user->phone === $phone && $user->whatsapp_verified_at)) {
    ...
    private function sendWhatsAppDirect(string $phone, string $message): void
    {
        ...
        if (!$token) {
            Log::info('No active WhatsApp token found', [
                'phone' => $phone
            ]);
            return;
        }
