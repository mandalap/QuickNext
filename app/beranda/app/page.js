...
import { 
  FiShoppingCart, FiPackage, FiBarChart2, FiLock, FiCloud, FiHeadphones,
  FiUsers, FiTrendingUp, FiCheck, FiChevronDown, FiChevronUp, FiSun, FiMoon,
  FiMenu, FiX, FiArrowUp, FiMessageCircle, FiPhone, FiMail, FiMapPin,
  FiFacebook, FiTwitter, FiInstagram, FiLinkedin, FiCreditCard, FiPrinter,
  FiSmartphone, FiDollarSign, FiShield, FiZap
} from 'react-icons/fi'
...
export default function LandingPage() {
      ...
      try {
        ...
        if (result.success && result.data) {
          ...
          const transformedPlans = result.data.map(plan => {
            ...
            const monthlyPrice = plan.prices?.find(p => p.duration_type === 'monthly') || plan.prices?.[0]
            const price = monthlyPrice ? Math.round(monthlyPrice.final_price) : 0
            ...
            if (Array.isArray(plan.features)) {
              ...
              featuresList = plan.features.map(feature => {
                // If feature is an object with 'feature' key (from backend model)
                if (typeof feature === 'object' && feature !== null) {
                  // Priority: feature.feature > feature.name > feature.text > etc
                  return feature.feature || feature.name || feature.text || feature.title || feature.description || String(feature)
                }
              ...
              try {
                ...
                if (Array.isArray(parsed)) {
                  ...
                  featuresList = parsed.map(feature => {
                    if (typeof feature === 'object' && feature !== null) {
                      return feature.feature || feature.name || feature.text || feature.title || feature.description || String(feature)
                    }
                    return String(feature)
            ...
            if (plan.max_outlets > 0) {
              const outletText = `${plan.max_outlets === -1 ? 'Unlimited' : plan.max_outlets} Outlet`
              if (!featuresList.includes(outletText)) {
                featuresList.unshift(outletText)
              }
            }
            if (plan.max_employees > 0) {
              const userText = `${plan.max_employees === -1 ? 'Unlimited' : plan.max_employees} User`
        ...
        setPricingPlans([
          {
            ...
            features: ['1 Outlet', '2 User Kasir', 'Kelola Produk', 'Laporan Basic', 'Support Email', 'Cloud Storage 5GB'],
            popular: false,
            cta: 'Mulai Sekarang'
          },
          {
            ...
            features: ['5 Outlet', 'Unlimited User', 'Kelola Produk', 'Laporan Advanced', 'Support Prioritas', 'Cloud Storage 50GB', 'Integrasi QRIS', 'Multi Gudang'],
            popular: true,
            cta: 'Paling Populer'
          },
          {
            ...
            features: ['Unlimited Outlet', 'Unlimited User', 'Semua Fitur Pro', 'Laporan Custom', 'Dedicated Support', 'Cloud Storage Unlimited', 'API Access', 'White Label', 'Training Gratis'],
            popular: false,
            cta: 'Hubungi Kami'
          }
        ])
  ...
  const features = [
    {
      ...
      title: 'Multi-Outlet',
      description: 'Kelola beberapa cabang dalam satu platform terpadu dengan sinkronisasi real-time'
    },
    {
      icon: <FiPackage className="w-8 h-8" />,
      title: 'Kelola Produk',
      description: 'Manajemen produk lengkap dengan kategori, varian, dan gambar produk'
    },
    {
      icon: <FiBarChart2 className="w-8 h-8" />,
      title: 'Manajemen Stok',
      description: 'Pantau stok secara real-time dengan notifikasi otomatis untuk stok menipis'
    },
    {
      icon: <FiTrendingUp className="w-8 h-8" />,
      title: 'Laporan Realtime',
      description: 'Dashboard analitik lengkap dengan laporan penjualan, laba rugi, dan grafik'
    },
    {
      icon: <FiUsers className="w-8 h-8" />,
      title: 'Akses Per User',
      description: 'Atur hak akses kasir dan admin dengan sistem role yang fleksibel'
    },
    {
      icon: <FiLock className="w-8 h-8" />,
      title: 'Keamanan Data',
      description: 'Enkripsi data tingkat enterprise dengan backup otomatis setiap hari'
    },
    {
      icon: <FiCloud className="w-8 h-8" />,
      title: 'Cloud Backup',
      description: 'Data tersimpan aman di cloud dengan akses dari mana saja, kapan saja'
    },
    {
      icon: <FiHeadphones className="w-8 h-8" />,
      title: 'Support 24/7',
      description: 'Tim support siap membantu Anda kapan saja via chat, telepon, atau email'
    }
  ]
  ...
  const whyChooseUs = [
    ...
    },
    {
      ...
      description: 'Digunakan oleh 10,000+ bisnis di Indonesia'
    },
    {
  ...
  const testimonials = [
    {
      name: 'Budi Santoso',
      position: 'Pemilik Warung Kopi Nusantara',
      image: 'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=100&h=100&fit=crop',
      text: 'QuickKasir mengubah cara saya mengelola bisnis. Sekarang saya bisa pantau 3 cabang dari mana saja!',
      rating: 5
    },
    {
      name: 'Siti Rahmawati',
      position: 'Owner Toko Fashion Elegan',
      image: 'https://images.unsplash.com/photo-1438761681033-6461ffad8d80?w=100&h=100&fit=crop',
      text: 'Manajemen stok jadi mudah banget. Notifikasi stok habis sangat membantu supaya tidak kehabisan produk populer.',
      rating: 5
    },
    {
      name: 'Ahmad Hidayat',
      position: 'Manager Restoran Seafood Bay',
      image: 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=100&h=100&fit=crop',
      text: 'Laporan real-time membantu saya membuat keputusan bisnis yang tepat. Sangat recommended!',
      rating: 5
    },
    {
      name: 'Dewi Lestari',
      position: 'Pemilik Salon Cantik',
      image: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=100&h=100&fit=crop',
      text: 'Support-nya luar biasa! Setiap ada masalah langsung dibantu dengan cepat dan ramah.',
      rating: 5
    },
    {
      name: 'Rudi Wijaya',
      position: 'Owner Mini Market Berkah',
      image: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=100&h=100&fit=crop',
      text: 'Harga sangat terjangkau dibanding kompetitor. Fiturnya lengkap dan mudah digunakan karyawan.',
      rating: 5
    },
    {
      name: 'Maya Kusuma',
      position: 'Pemilik Bakery Delicious',
      image: 'https://images.unsplash.com/photo-1487412720507-e7ab37603c6f?w=100&h=100&fit=crop',
      text: 'Sejak pakai QuickKasir, omzet naik 40%! Proses checkout cepat dan pelanggan puas.',
      rating: 5
    }
  ]
  ...
  const integrations = [
    { icon: <FiCreditCard className="w-8 h-8" />, name: 'QRIS', description: 'Pembayaran digital' },
    { icon: <FiPrinter className="w-8 h-8" />, name: 'Thermal Printer', description: 'Print struk otomatis' },
    { icon: <FiSmartphone className="w-8 h-8" />, name: 'Mobile App', description: 'Android & iOS' },
    { icon: <FiCreditCard className="w-8 h-8" />, name: 'EDC Machine', description: 'Debit & Credit Card' }
  ]
  ...
  return (
    <div className={`min-h-screen ${theme === 'dark' ? 'dark' : ''}`}>
      <div className="bg-background text-foreground transition-colors duration-300">
        ...
        >
          <div className="container mx-auto px-4">
            ...
            {mobileMenuOpen && (
              ...
              >
                <button onClick={() => scrollToSection('features')} className="block w-full text-left py-2 hover:text-primary transition-colors">
                  Fitur
                </button>
                <button onClick={() => scrollToSection('pricing')} className="block w-full text-left py-2 hover:text-primary transition-colors">
                  Harga
                </button>
                <button onClick={() => scrollToSection('demo')} className="block w-full text-left py-2 hover:text-primary transition-colors">
                  Demo
                </button>
          ...
          <div className="container mx-auto">
            ...
            >
              ...
              <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
                QuickKasir dilengkapi fitur-fitur canggih yang dirancang khusus untuk meningkatkan efisiensi bisnis Anda
              </p>
            </motion.div>
        ...
        <section className="py-20 px-4 bg-gradient-to-b from-background to-emerald-50 dark:to-gray-900">
          <div className="container mx-auto">
            ...
            >
              ...
              <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
                Bergabunglah dengan ribuan bisnis yang sudah merasakan manfaatnya
              </p>
            </motion.div>
            ...
            >
              <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
                ...
                <div>
                  ...
                  <div className="text-muted-foreground">Transaksi per Hari</div>
                </div>
                <div>
              ...
              {loadingPlans ? (
                ...
                >
                  <Card className={`h-full relative overflow-hidden transition-all duration-300 ${
                    plan.popular ? 'border-2 border-emerald-500 dark:border-emerald-400 shadow-2xl shadow-emerald-500/20 hover:shadow-emerald-400/30 hover:scale-[1.02]' : 'hover:border-emerald-300 dark:hover:border-emerald-700'
                  }`}>
                    {plan.popular && (
                    ...
                    <CardContent className="space-y-3">
                      {plan.features && Array.isArray(plan.features) && plan.features.length > 0 ? (
                        plan.features.map((feature, i) => {
                          ...
                          let featureText = ''
                          if (typeof feature === 'object' && feature !== null) {
                            featureText = feature.feature || feature.name || feature.text || feature.title || feature.description || ''
                          } else {
                        ...
                        <p className="text-sm text-muted-foreground">Tidak ada fitur tersedia</p>
                      )}
                    </CardContent>
                    <CardFooter>
                      <Button 
        ...
        <section className="py-20 px-4 bg-gradient-to-b from-background to-emerald-50 dark:to-gray-900">
          <div className="container mx-auto">
            ...
            >
              ...
              <h2 className="text-3xl md:text-4xl font-bold mb-4">
                Apa Kata Mereka Tentang QuickKasir?
              </h2>
              <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
                Dengarkan pengalaman dari ribuan pebisnis yang sudah merasakan manfaatnya
              </p>
            </motion.div>
        ...
        <section className="py-20 px-4 bg-background">
          <div className="container mx-auto">
            ...
            >
              ...
              <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
                QuickKasir mendukung berbagai metode pembayaran dan perangkat untuk kemudahan bisnis Anda
              </p>
            </motion.div>
        ...
        {/* Demo Video Section */}
        <section id="demo" className="py-20 px-4 bg-gradient-to-b from-background to-emerald-50 dark:to-gray-900">
          <div className="container mx-auto">
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
            ...
            >
              ...
              <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
                Tonton video demo singkat untuk melihat betapa mudahnya menggunakan QuickKasir
              </p>
            </motion.div>
          ...
          <div className="container mx-auto text-center">
            ...
            >
              ...
              <p className="text-xl mb-8 opacity-90 max-w-2xl mx-auto">
                Bergabunglah dengan 10,000+ bisnis yang sudah berkembang bersama QuickKasir. Coba gratis selama 14 hari, tanpa kartu kredit!
              </p>
            ...
            <div className="grid md:grid-cols-4 gap-8 mb-12">
              ...
              <div>
                <div className="flex items-center space-x-2 mb-4">
                  ...
                  <span className="text-xl font-bold text-white">QuickKasir</span>
                </div>
                <p className="text-sm mb-4">
                  Solusi POS modern untuk semua jenis bisnis. Tingkatkan efisiensi dan produktivitas dengan teknologi terkini.
                </p>
                <div className="flex space-x-3">
                  <a href="#" className="w-8 h-8 bg-gray-800 rounded-full flex items-center justify-center hover:bg-emerald-500 hover:scale-110 active:bg-emerald-600 transition-all duration-300">
